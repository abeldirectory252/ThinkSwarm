...
Provides simple inter-process communication between the Flask backend and simulation scripts.

Implements a simple command/response pattern using the filesystem:
1. Flask writes commands into the `commands/` directory
2. The simulation process polls the commands directory, executes commands and writes responses to `responses/`
...
logger = get_logger('ThinkSwarm.simulation_ipc')
...
class SimulationIPCClient:
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Optional platform filter
                ...
                - None: interview on both platforms for dual-world sim, or the active platform for single-platform sim
            timeout: timeout in seconds

        Returns:
            IPCResponse; the `result` field contains the interview output
        """
        args = {
            "agent_id": agent_id,
            "prompt": prompt
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: list of interviews, each item: {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: default platform (optional; per-item `platform` overrides this)
                - "twitter": default to Twitter
                - "reddit": default to Reddit
                - None: in dual-world sim each agent is interviewed on both platforms
            timeout: timeout in seconds
    ...
    def send_close_env(self, timeout: float = 30.0) -> IPCResponse:
        """Send command to close the simulation environment
...
class SimulationIPCServer:
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        ...
        # Collect and sort command files by modification time
        command_files = []
