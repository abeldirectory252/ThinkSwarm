  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">ThinkSwarm</div>
      </div>
...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds are provided, pass them via query parameters
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful shutdown fails, attempt a forced stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(t('log.closeSimEnvException', { error: closeErr.message }))
        // If graceful shutdown throws, attempt a forced stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failure checking environment status should not block the flow
